...
def generate_df_affect_by_n_days(series, n, index=False):
    if len(series) <= n:
        return Exception('The Length of series is %d,while affect by (n=%d).' % (len(series), n))
    df = DataFrame()
    for i in range(n):
...
def read_data(column='收盘价', n=30, all_too=True, index=False, train_end=-300):
    df = read_csv('000001.csv', index_col=0, encoding='gbk')
    df = df.drop('股票代码', axis=1)
    df = df.drop('名称', axis=1)
    df.index = list(
        map(lambda x: datetime.strptime(x, '%Y/%m/%d'), df.index))
...
df, df_all, df_index = read_data('收盘价', n=n, train_end=train_end)
...
plot(df_index[train_end:],
     generate_data_test[:], label='generate-train')
legend()
show()
